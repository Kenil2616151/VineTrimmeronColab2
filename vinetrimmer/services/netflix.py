...
class Netflix(BaseService):
    ...
    *MPL: FHD with Android L3, sporadically available with ChromeCDM
    ...
    @click.option("-p", "--profile", type=click.Choice(["MPL", "HPL", "MPL+HPL"], case_sensitive=False),
                  default="MPL+HPL",
    ...
    def get_tracks(self, title):
        if self.vcodec == "H264":
            ...
            self.config["profiles"]["video"]["H264"]["MPL+HPL"] = (
            ...
            for profile in profiles:
                if self.profile == "MPL+HPL" and profile == "MPL" and self.cdm.device.type == LocalDevice.Types.CHROME:
